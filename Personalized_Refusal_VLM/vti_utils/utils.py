...
def get_all_datasets(args):
    if args.data.dataset_name == "ScienceQA":
        ...
        in_domain = []
        in_domain.append(args.data.subject)
        out_of_domain = ['Math', 'Geography', 'Art_Theory', 'Psychology', 'Computer_Science']
        out_of_domain = [domain for domain in out_of_domain if domain not in in_domain]
    ...
    for i in range(args.num_test):
        ...
        if args.data.dataset_name == "ScienceQA":
            ...
            in_img_test = sample_in[i]['image_1']
...
def get_activations_teacher_enforce(model, inputs_text, image, processor, system_prompt=False):
    if system_prompt:
        h_all = []
        with torch.no_grad():
            for example_id in tqdm(range(len(inputs_text)), desc="Getting teacher enforced activations", total=len(inputs_text)):
    ...
    with torch.no_grad():
        for example_id in tqdm(range(len(inputs_text)), desc="Getting activations", total=len(inputs_text)):
            ...
            for style_id in range(len(text_list)):
                ...
                inputs = processor(text=text, images=image[example_id].convert("RGB"), return_tensors="pt")
                ...
                if 'idefics3-' in cfg.model_name.lower():
                    ...
                    assistant_start = assistant_positions[-1].item() + 3
