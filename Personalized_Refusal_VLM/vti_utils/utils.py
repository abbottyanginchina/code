...
def get_all_datasets_filter(args):
    ...
    dataset = load_dataset(f"/gpu02home/jmy5701/gpu/data/ScienceQA", memory_map=True)["train"].filter(lambda e: e["image"] is not None)
    ...
    if 'llama' in args.model_path.lower():
        sys_prompt = "<|image|><|begin_of_text|>" + sys_prompt
            ...
            for style_id in range(len(text_list)):
                if system_prompt:
                    conversation = [
                        ...
                        "role": "system",
                        "content": [
                            {"type": "text", "text": "Say 'I cannot answer that question.' directly if you refuse to answer."},
                            ],
                        },
                        {
                            "role": "user",
