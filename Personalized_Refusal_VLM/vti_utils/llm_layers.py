...
class VTILayer(nn.Module):
    ...
    def forward(self, x):
        if self.vti_direction is not None:
            ...
            x = F.normalize(F.normalize(x.float(),dim=-1) +  0.1 * y, dim=-1) * norm    # x: torch.Size([1, 595, 4096]) y: torch.Size([4096]) norm: torch.Size([1, 595, 1])
...
def add_multiple_layers(model: PreTrainedModel, vti_directions: Tensor, alpha: list, layer_indices: list[int], cfg):
