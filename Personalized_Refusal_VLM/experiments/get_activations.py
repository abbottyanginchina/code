...
def eval_model(args):
    ...
    with torch.no_grad():
        ...
        if 'qwen-' in cfg.model_name.lower():
            with_sys_in_train_activations = process(get_activations_qwen(model, with_sys_in_train_text, in_train_images, tokenizer, system_prompt=False))
            with_sys_out_train_activations = process(get_activations_qwen(model, with_sys_out_train_text, out_train_images, tokenizer, system_prompt=False))
            without_sys_in_train_activations = process(get_activations_qwen(model, without_sys_in_train_text, in_train_images, tokenizer, system_prompt=False))
            without_sys_out_train_activations = process(get_activations_qwen(model, without_sys_out_train_text, out_train_images, tokenizer, system_prompt=False))

            # 1. 加 system prompt 的 others（对应 h_c(Image_{others} + system_prompt)）
            with_sys_image_others_activations = process(
                get_activations_qwen(model, [""] * len(out_train_images), out_train_images, tokenizer, system_prompt=True)
            )
            # 2. 不加 system prompt 的 biology（对应 h_c(Image_{biology} + "None")）
            without_sys_image_biology_activations = process(
                get_activations_qwen(model, [""] * len(in_train_images), in_train_images, tokenizer, system_prompt=False)
            )

            in_test_activations = process(get_activations_qwen(model, in_test_text, in_test_images, tokenizer, system_prompt=False))
            out_test_activations = process(get_activations_qwen(model, out_test_text, out_test_images, tokenizer, system_prompt=False))

            image_in_test_activations = process(
                get_activations_qwen(model, [""] * len(in_test_images), in_test_images, tokenizer, system_prompt=False)
            )
            image_out_test_activations = process(
                get_activations_qwen(model, [""] * len(out_test_images), out_test_images, tokenizer, system_prompt=False)
            )
        else:
