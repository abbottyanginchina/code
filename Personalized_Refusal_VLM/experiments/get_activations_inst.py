...
from vti_utils.utils import get_activations_blip, get_activations, get_all_datasets_filter, get_all_datasets, get_activations, get_activations_inst, get_activations_blip_inst
...
def eval_model(cfg):
    ...
    with torch.no_grad():
        if 'instructblip-' in model_path.lower():
            with_sys_out_train_activations = process(get_activations_blip_inst(cfg, model, with_sys_out_train_text, out_train_images, processor, system_prompt=True))
            with_sys_in_train_activations = process(get_activations_blip(model, with_sys_in_train_text, in_train_images, processor, system_prompt=False))
            ...
            with_sys_out_train_activations = process(get_activations_inst(cfg, model, without_sys_out_train_text, out_train_images, processor, system_prompt=True))
            with_sys_in_train_activations = process(get_activations(model, without_sys_in_train_text, in_train_images, processor, system_prompt=False))
    ...
    save_path = os.path.join(cfg.output_dir, f"output_{cfg.model_name}_{cfg.data.dataset_name}_{cfg.data.subject}/activations/")
