...
def eval_model(cfg):
    ...
    if 'qwen-' in model_path.lower():
        ...
        processor.tokenizer._tokenizer.set_threads(1)
    ...
    with torch.no_grad():
        with_sys_in_train_activations = process(get_activations_inst(model, with_sys_in_train_text, in_train_images, processor, system_prompt=True))
        with_sys_out_train_activations = process(get_activations_inst(model, with_sys_out_train_text, out_train_images, processor, system_prompt=True))
        without_sys_in_train_activations = process(get_activations_inst(model, without_sys_in_train_text, in_train_images, processor, system_prompt=False))
        without_sys_out_train_activations = process(get_activations_inst(model, without_sys_out_train_text, out_train_images, processor, system_prompt=False))
