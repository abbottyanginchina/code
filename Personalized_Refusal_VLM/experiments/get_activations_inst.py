...
from vti_utils.utils import get_activations_blip, get_activations, get_all_datasets_filter, get_all_datasets, get_activations, get_activations_inst
...
def eval_model(cfg):
    ...
    with torch.no_grad():
        with_sys_in_train_activations = process(get_activations_inst(cfg, model, with_sys_in_train_text, in_train_images, processor, system_prompt=False))
        ...
        without_sys_in_train_activations = process(get_activations_inst(cfg, model, without_sys_in_train_text, in_train_images, processor, system_prompt=False))
        without_sys_out_train_activations = process(get_activations_inst(cfg, model, without_sys_out_train_text, out_train_images, processor, system_prompt=True))
        ...
        in_test_activations = process(get_activations_inst(cfg, model, in_test_text, in_test_images, processor, system_prompt=False))
        out_test_activations = process(get_activations_inst(cfg, model, out_test_text, out_test_images, processor, system_prompt=True))
        image_in_test_activations = process(
            get_activations_inst(cfg, model, [""] * len(in_test_images), in_test_images, processor, system_prompt=False)
        )
