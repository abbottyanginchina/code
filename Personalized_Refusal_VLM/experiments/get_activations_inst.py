...
def eval_model(cfg):
    ...
    with torch.no_grad():
        ...
        image_in_test_activations = process(
            get_activations(model, [""] * len(in_train_images), in_test_images, processor, system_prompt=False)
        )
        image_out_test_activations = process(
            get_activations(model, [""] * len(in_train_images), out_test_images, processor, system_prompt=False)
        )
