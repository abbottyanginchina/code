...
def load_activations(cfg, layer):
    bio_x = torch.load(f"./output/activations/without_sys_in_train_activations_{cfg.model_name}.pt", weights_only=False)[:, layer, :].to(device, dtype=torch.float64)
    oth_x = torch.load(f"./output/activations/without_sys_out_train_activations_{cfg.model_name}.pt", weights_only=False)[:, layer, :].to(device, dtype=torch.float64)
    bio_target = bio_x
    oth_target = torch.load(f"./output/activations/with_sys_out_train_activations_{cfg.model_name}.pt", weights_only=False)[:, layer, :].to(device, dtype=torch.float64)
...
def inference(cfg, model, layer):
    ...
    image_others_x_test = torch.load(f"./output/activations/image_out_test_activations_{cfg.model_name}.pt", weights_only=False)[:, layer, :].to(device).double()
    image_biology_x_test = torch.load(f"./output/activations/image_in_test_activations_{cfg.model_name}.pt", weights_only=False)[:, layer, :].to(device).double()
    ...
    torch.save(image_pred_other, f"./output/activations/image_pred_other_layer{layer}_{cfg.model_name}.pt")
    torch.save(image_pred_biology, f"./output/activations/image_pred_biology_layer{layer}_{cfg.model_name}.pt")
...
def main(cfg):
    for layer in range(cfg.start_layer, cfg.end_layer): 
        ...
        model.load_state_dict(torch.load(f"./output/models/steering_model_layer{layer}_{cfg.model_name}.pt", weights_only=False).state_dict())
        model.eval()
