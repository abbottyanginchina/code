...
def eval_model(args):
    ...
    # SVD
    # import pdb; pdb.set_trace()
    V_dict = compute_layerwise_V_k(oth_target, oth_x, device, k=64)
    ...
    for img_id in range(len(out_test_images)):
        ...
        add_multiple_layers(V_dict, model, torch.stack([refusal_all[img_id]],dim=1).cuda(), alpha = [cfg.alpha_text], layer_indices = target_layers, cfg = cfg)
