...
def eval_model(args, output_dir):
    ...
    vision_steering_list1 = []
    vision_steering_list2 = []
    for layer in range(0, args.max_layer): # Must start from 1
        ...
        vec1 = torch.load(path1, weights_only=False)
        vec2 = torch.load(path2, weights_only=False)
        vision_steering_list1.append(vec1)
        vision_steering_list2.append(vec2)
    vision_refusal_all = torch.stack(vision_steering_list1, dim=1)
    vision_biology_all = torch.stack(vision_steering_list2, dim=1)
    ...
    save_dir = f"../results/vision_{cfg.model_name}_{cfg.data.dataset_name}_{cfg.data.subject}/refusal_scores/"
